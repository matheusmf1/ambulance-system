...
export class ServiceOrder {
  ...
  addServiceOrderToFirebase = async () => {
    try {
      ...
      await setDoc( doc( db, `users/${auth.currentUser.uid}/orcamento_venda_serviceOrder`, `${this.data['id']}` ), this.data );
  ...
  getServiceOrderFromFirebase = async () => {
    ...
    try {
      const docRef = doc( db, `users/${auth.currentUser.uid}/orcamento_venda_serviceOrder`, `${this.id}` );
  ...
  deleteServiceOrderFromFirebase = async () => {
    try {
      await deleteDoc( doc( db, `users/${auth.currentUser.uid}/orcamento_venda_serviceOrder`, `/${this.id}` ) );
      return true
